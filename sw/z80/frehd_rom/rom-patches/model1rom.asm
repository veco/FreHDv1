...
;; ROM: 	ld   sp,0600h			;0066 
;; ROM:  	ld   a,(37ech)			;0069 
;; ROM:  	inc  a					;006c 
;; ROM:  	cp   02h              	;006d 
;; ROM:  	jp,nc   0000h         	;006f 
;; ROM: 	jp   06cch            	;0072 
;; ROM:   	ld   de,4080h         	;0075 
;; ROM:		ld   hl,18f7h	     	;0078	

	ORG	0066h
	jp		0000h			;	0066 start  here if reset button pressed
	call	01c9h			; 	0069 clear screen
	call	0060h			;	006c call delay routine, trashes bc 
	ret						;	006c delay completed
	db		0,0,0,0,0		;   006d
	IF $ > 0075h
	ERROR "Code too big"
	ENDIF
...
;; now 29 bytes free here !
	...
	ORG 00ffh
hd_boot:	
	call 0069h				; 00ff  3 sec delay
	ld	a,ROM_MODEL_1		; 0102  ROM parameter to FreHD. 1 for Model1.
	out	(WRITEROM),a		; 0104  2
	in	a,(READROM)			; 0106  2
	cp	0FEh				; 0108  FE is FreHD present and valid code!
	ret	nz					; 010a  return if no FreHD or old firmware
	ld	hl,ROM_LOAD		    ; 010b  load more "rom" from FreHD
	ld	bc,READROM		    ; 010e
	inir				    ; 0111
	jp	ROM_LOAD			; 0113
	db 0,0,0,0,0			; 0116
	...
	ORG 0471h
	jr	047dh			; skip upper case conversion
